...
# rebuild by Hiepnh21
# Update by Hiepnh21 HT2304200700
    ...
    for row in logMsg['hits']['hits']:
        if "/share/hni/" in row['_source']['source']:
            ...
            if  ("not available anymore" in message) or ("loop broken" in message) or ("No Connection could be retrieved" in message) or \
                ...
                ("refreshing mediaserver" in message) or ("replication-check" in message) or ("Cannot persist" in message) or ("Could not reconnect to link aareswitch" in message) or \
                ("ACK per second" in message) or ("INVITE per second" in message) or ("topstop limit" in message):
...
#                                    u'message': u'2020-05-20-11:07:16.079 17 sc2420594310890 [TOPSTOP][ALARM][INFO] tenant FPT-HCM: topstop limit nearly reached for account SGVO12565_HCM, number 02873089968',
#                                    # u'message': u'2020-07-31-14:42:26.907 [WARN ]            TrafficShaper (#5) start shaping: 500 ACK per second',
...
#                                    u'source': u'/share/sc01/servicecenter/alarm.log',
